...
// for(let i=0; i<dateRanges.length; i++) {
//     const newExplorerWeekly = new ExploreTrendRequest()
//     newExplorerWeekly.addKeyword(keyword)
//         .between(dateRanges[i].initialDate, dateRanges[i].finalDate)
//         .download().then( file => {
//             const fileToCSV = new objToCSV(file)
//             return fileToCSV.toDisk(`./csv/weekly-${(dateRanges[i].initialDate).substr(0,4)}.csv`)
//         })
// }

// /*
//     Second, information will be collected daily, searching each six months
// */
// const months = 6

// initialDate = new Date(Date.parse(iniDate))
// initialDate.setDate(initialDate.getDate() + 1)
...
// for(let i=0; i<dateRanges.length; i++) {
//     const newExplorerDaily = new ExploreTrendRequest()
//     newExplorerDaily.addKeyword(keyword)
//         .between(dateRanges[i].initialDate, dateRanges[i].finalDate)
//         .download().then( file => {
//             const fileToCSV = new objToCSV(file)
//             return fileToCSV.toDisk(`./csv/daily-${dateRanges[i].initialDate}_to_${dateRanges[i].finalDate}.csv`)
//         })
// }
